...
import cryptoAid from "../abi/cryptoAid.json";
import {
...
export const createCampaign = async (
  ...
  const config = await prepareWriteContract({
    ...
    args: [parseEther(amount), title, description],
  });
 ...
 * @return {Promise<{ result: any; hash: string; }>} A Promise that resolves to an object containing the result and hash of the release.
 */
export const releaseDonation = async (fundId: number) => {
  const config = await prepareWriteContract({
    address: process.env.NEXT_PUBLIC_CONTRACT_ADDRESS as `0x${string}`,
    abi: cryptoAid.abi,
